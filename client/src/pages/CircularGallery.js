...
class App {
  ...
  updateClickHandlers() {
    ...
    this.medias.forEach((media, index) => {
      ...
      if (index < this.items.length && this.items[index]) {
        ...
        const planeHeight = this.viewport ? (this.viewport.height * (900 * scale)) / this.screen.height : 200;
        const planeWidth = this.viewport ? (this.viewport.width * (700 * scale)) / this.screen.width : 150;
        ...
        handler.style.left = `${(x - planeWidth / 2)}px`;
        handler.style.top = `${(y - planeHeight / 2)}px`;
        handler.style.width = `${planeWidth}px`;
        handler.style.height = `${planeHeight}px`;
        
        // Add click event
        handler.addEventListener('click', () => {
          // Navigate to product detail page
          if (this.items[index] && this.items[index].id) {
            window.location.href = `/product/${this.items[index].id}`;
          }
        });
  ...
  update() {
    // Handle auto-scroll if enabled and no recent user interaction
    if (this.autoScroll && !this.isDown && (Date.now() - this.lastInteractionTime > this.interactionTimeout)) {
...
export default function CircularGallery({
  ...
  textColor = '#ffffff',
  borderRadius = 0.04, // Use borderRadius=0.04 to match card design language
  ...
  autoScroll = true,
  autoScrollSpeed = 0.1,
  height = 500 // Add height parameter to make cards smaller
}) {
